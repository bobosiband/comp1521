...
find_student_by_id__loop__body:
	...
	add		$t1, $t1, $t2
	lw		$t3, 0($t1)
	bne		$t3, $s0, find_student_by_id__loop__step
studen_found:
	move 	$v0, $t1
...
update_student_mark__body:
	...
	la 		$a0, update_student_mark__id_prompt     #"Please enter the student ID: "
	li		$v0, 4
	syscall
	li		$v0, 5
	syscall
	...
	bne		$t0, NULL, student_condtion_true
	la		$a0, update_student_mark__invalid_id      #"Student not found in class!\n"
	li		$v0, 4
	syscall
	b		update_student_mark__epilogue
student_condtion_true:
	la		$a0, update_student_mark__mark_prompt    # "Please enter the student mark: "
	li		$v0, 4
	syscall
	li		$v0, 5
	syscall
	add		$t1, $t0, STRUCT_STUDENT_MARK_OFFSET
	sw		$v0, 0($t1)
...
print_report__loop__body:
	...
	add		$t1, $t1, STRUCT_STUDENT_MARK_OFFSET
	lw		$t3, 0($t1)
	bne		$t3, UNKNOWN_MARK, mark_is_available
mark_is_unkown:
